...
class ClockProPlus:
    ...
    def evict(self):
        ...
        while not evicted:
            ...
            if page in self.demoted_cold_pages and ref > 0:
    ...
    def decrease_capacity(self):
        ...
        self.capacity_hot = max(0, self.capacity_hot - min(1, c_d/c_n))

    #only to be called when reference bit set on a redsident cold page demoted from hot page status
    def increase_capacity(self):
        c_n = len(self.non_res_pages)
        c_d = len(self.demoted_cold_pages)
        self.capacity_hot = min(self.max_capacity_hot, self.capacity_hot + min(1, c_n/c_d))
